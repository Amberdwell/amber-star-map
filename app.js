    ...
    filtered.forEach(loc => {
        ...
        const popupContent = `
            <div class="luxury-popup-card" style="width: 320px;">
                <div class="popup-img-container" style="height: 150px; overflow: hidden;"><img src="${loc.image}" style="width: 100%; height: 100%; object-fit: cover;"></div>
                <div class="popup-content-body" style="padding: 12px;">
                    <h2 style="margin: 0 0 5px 0; font-size: 16px;">${loc.name}</h2>
                    <div style="margin-bottom: 8px;">
                        <span style="color: #D4AF37; font-weight: bold;">${getStars(loc.score)}</span> 
                        <span style="font-size: 12px; color: #666;">(Level ${level})</span>
                    </div>
                    <div style="font-size: 11px; color: #888; text-transform: uppercase; margin-bottom: 10px;">Audit Score: ${loc.score} / 150</div>
                    <p style="font-size: 13px; margin-bottom: 10px;">${loc.description}</p>
                    <a href="${loc.website.startsWith('http') ? loc.website : 'https://'+loc.website}" target="_blank" style="display: block; background: #333; color: #fff; text-align: center; padding: 8px; text-decoration: none; font-size: 12px;">Official Website</a>
                </div>
            </div>
        `;
